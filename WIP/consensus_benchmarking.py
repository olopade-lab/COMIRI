...
def consensus(row):
                    ...
                    if list_len.count(max(list_len)) == 1:
                        ...
                        for program in ["MiXCR", "TRUST3", "TRUST4", "CATT"]:
                            if isinstance(row["count_"+program], list):
                                all_count += [int(float(count)) for count in row["count_"+program]]
                        for mode in modes:
    ...
    for program in ["MiXCR", "TRUST3", "TRUST4", "CATT"]:
        if isinstance(row["count_"+program], list):
            all_count += [sum([int(float(count)) for count in row["count_"+program]])]
...
def quorum(row, list_algorithms, num_votes):
    return ["Y" if isinstance(row["cdr3dna_"+algorithm], list) else "N" for algorithm in list_algorithms].count("Y") >= num_votes

for sample in ["FZ-116"]:
    for receptor in ["BCR-heavy"]:
        df_merged = pd.read_csv(f"/Users/jbreynier/results_benchmarking/{sample}_{receptor}.csv")
        ...
        df_merged = df_merged[df_merged.apply(lambda cdr3dna_program: quorum(cdr3dna_program, ["MiXCR", "TRUST3", "TRUST4", "CATT"], 1), axis=1)]
        ...
        df_consensus.to_csv(f"/Users/jbreynier/results_benchmarking/{sample}_{receptor}_consensus_q1.csv", index=False)