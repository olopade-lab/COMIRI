...
def merge_results(sample_dir, attribute):
    ...
    for program in os.listdir(sample_dir):
        if program == "MiXCR":
            print("MiXCR")
            try:
                df_MiXCR = pd.read_csv(sample_dir+"/MiXCR/TCR/"
                                        +sample_name+"_"+attribute+"_clones.txt",
                                        sep="\t",
                                        usecols=["cloneCount",
                                                "allVHitsWithScore",
                                                "allDHitsWithScore",
        ...
        if program == "TRUST3":
            print("TRUST3")
            try:
                with open(sample_dir+"/TRUST3/TCR/"+sample_name+dict_TRUST3[attribute], "r") as fasta_file:
        ...
        if program == "CATT":
            ...
            if attribute == "TRA":
                try:
                    df_CATT = pd.read_csv(sample_dir+"/CATT/"+attribute+"/"+sample_name+".TRA.CDR3.CATT.csv",
                                                sep=",",
                                                usecols=['AAseq',
                                                        'NNseq',
                                                        'Vregion',
...
for sample_dir in glob.glob("/scratch/jbreynier/COMIRI_outputs/RNASeq_SPX81*/"):
